...
import { getActivities, addActivity, deleteActivity } from "../lib/firebase";
...
export default function ActivitiesPage() {
  ...
  const [form, setForm] = useState({ type: "Email", subject: "", contact: "", company: "", notes: "" });
  ...
  async function save() {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ type: "Email", subject: "", contact: "", company: "", notes: "" });
      load();
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <div className="flex gap-2 mb-4">
        {["All", ...TYPES].map((t) => (
          <button key={t} className="px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">{t}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400 text-sm">No activities yet. Log your first email, call, or meeting.</div>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {a.type} {a.contact && `· ${a.contact}`} {a.company && `· ${a.company}`}
                      </p>
