...
// ─── Notifications ───────────────────────────────────────────
// Creates a notification for a specific salesperson
export async function createNotification({ toSalesperson, type, title, body, refId = "", refType = "" }) {
  if (!toSalesperson || toSalesperson === "Unassigned") return;
  return addDoc(collection(db, "notifications"), {
    toSalesperson,   // salesperson name this notification is for
    type,            // "contact_assigned" | "deal_assigned"
    title,
    body,
    refId,           // id of the contact or deal
    refType,         // "contact" | "deal"
    read: false,
    createdAt: serverTimestamp(),
  });
}

// Get notifications for a specific salesperson (unread first, last 50)
export async function getNotifications(salesperson) {
  try {
    if (!salesperson) return [];
    const snap = await getDocs(collection(db, "notifications"));
    return snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .filter(n => n.toSalesperson === salesperson)
      .sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0))
      .slice(0, 50);
  } catch (e) { console.error("getNotifications error:", e); return []; }
}

// Mark all notifications as read for a salesperson
export async function markAllNotificationsRead(salesperson) {
  try {
    const snap = await getDocs(collection(db, "notifications"));
    const unread = snap.docs.filter(d =>
      d.data().toSalesperson === salesperson && !d.data().read
    );
...
export async function markNotificationRead(id) {
  try { return updateDoc(doc(db, "notifications", id), { read: true }); }
